...
func (cmd *RunCommand) WireDynamicFlags(commandFlags *flags.Command) {
	...
	var eventStoresGroup *flags.Group
	...
	for i := 0; i < len(groups); i++ {
		...
		if metricsGroup != nil && credsGroup != nil && authGroup != nil && eventStoresGroup != nil {
			break
		}
...
func (cmd *RunCommand) constructMembers(
	...
	backendComponents, err := cmd.backendComponents(logger, backendConn, lockFactory, secretManager, eventStore)
	if err != nil {
		return nil, err
	}

	gcComponents, err := cmd.gcComponents(logger, gcConn, lockFactory, eventStore)
	if err != nil {
		return nil, err
	}
