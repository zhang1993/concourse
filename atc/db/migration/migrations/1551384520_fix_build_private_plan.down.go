...
func (runner *migrations) Down_1550160079(db *sql.DB) error {
	...
	for _, build := range builds {
		...
		decrypted, err := runner.Strategy.Decrypt(build.plan.String, noncense)
		if err != nil {
			return err
		}
		...
		encrypted, newnonce, err := runner.Strategy.Encrypt(fixed)
		if err != nil {
			return err
		}
