...
func (r *resourceConfigScope) SaveVersions(versions []atc.Version) error {
	return saveVersions(context.TODO(), r.conn, r.ID(), versions)
}

func saveVersions(ctx context.Context, conn Conn, rcsID int, versions []atc.Version) error {
	tx, err := conn.Begin()
	if err != nil {
		return err
	}
	...
	for _, version := range versions {
		newVersion, err := saveResourceVersion(ctx, tx, rcsID, version, nil)
		if err != nil {
			return err
		}
...
func saveResourceVersion(ctx context.Context, tx Tx, rcsID int, version atc.Version, metadata ResourceConfigMetadataFields) (bool, error) {
	versionJSON, err := json.Marshal(version)
	if err != nil {
		return false, err
	}
