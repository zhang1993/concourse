...
var _ = Describe("VersionsDB", func() {
	...
	Describe("SuccessfulBuilds", func() {
		...
		Context("with a page of filler and then rerun builds created after their original builds", func() {
			...
			BeforeEach(func() {
				var err error
				build1Succeeded, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build1Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build2Failed, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build2Failed, db.BuildStatusFailed)
				Expect(err).ToNot(HaveOccurred())

				build3Succeeded, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build3Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build4Rerun2Succeeded, err = buildCreator.RerunBuild(defaultJob, build2Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build4Rerun2Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build5Rerun2Succeeded, err = buildCreator.RerunBuild(defaultJob, build2Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build5Rerun2Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build6Succeeded, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build6Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("with a rerun build of a failed build on the page limit boundary", func() {
			...
			BeforeEach(func() {
				var err error
				build1Failed, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build1Failed, db.BuildStatusFailed)
				Expect(err).ToNot(HaveOccurred())
				...
				build6Rerun1Succeeded, err = buildCreator.RerunBuild(defaultJob, build1Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build6Rerun1Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("with a rerun build of a succeeded build on the page limit boundary", func() {
			...
			BeforeEach(func() {
				...
				build6Rerun1Succeeded, err = buildCreator.RerunBuild(defaultJob, build1Succeeded)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build6Rerun1Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("with multiple reruns of the same build crossing the page limit boundary", func() {
			...
			BeforeEach(func() {
				...
				build7Rerun1Succeeded, err = buildCreator.RerunBuild(defaultJob, build1Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build7Rerun1Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build8Rerun1Succeeded, err = buildCreator.RerunBuild(defaultJob, build1Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build8Rerun1Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
			})
	...
	Describe("UnusedBuilds", func() {
		...
		Context("with one build, which was last used", func() {
			...
			BeforeEach(func() {
				var err error
				cursorBuild, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())

				err = finishBuild(cursorBuild, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("with some older builds and some newer builds and some reruns of the cursor", func() {
			...
			BeforeEach(func() {
				...
				for i := 0; i < pageLimit; i++ {
					build, err := buildCreator.RerunBuild(defaultJob, cursorBuild)
					Expect(err).ToNot(HaveOccurred())
		...
		Context("when the last used build was a rerun and there is another rerun before and after it", func() {
			...
			BeforeEach(func() {
				...
				build6Rerun2Succeeded, err = buildCreator.RerunBuild(defaultJob, build2Failed)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build6Rerun2Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())

				build7Succeeded, err = buildCreator.CreateBuild(defaultJob)
				Expect(err).ToNot(HaveOccurred())
				err = finishBuild(build7Succeeded, db.BuildStatusSucceeded)
				Expect(err).ToNot(HaveOccurred())
			})
