				...
				Context("when the build is successfully scheduled", func() {
					...
					Context("when all resources are checked after build create time or pinned", func() {
						BeforeEach(func() {
							...
							job.ConfigReturns(atc.JobConfig{PlanSequence: atc.PlanSequence{{Get: "input-1", Resource: "some-resource"}, {Get: "input-2", Resource: "other-resource"}}}, nil)
